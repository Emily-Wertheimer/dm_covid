...
state_pop <- as.data.frame(state_pops2)
trackerNew <- merge(tracker,state_pops2,by.x ="state") # trackerNew has cols for state pop & death rates
...
# GLM model 1: risk.mon ~ death rate + testing + hospitalizations + age (using defaults)
...
# GLM model 2: ambig_corr.mon ~ death rate + testing + hospitalizations + age
...
# model 4: ambig_corr.med ~ death rate + testing + hospitalizations + age
...
model6GLM <- glm(modData$data.all.ambig1_react_state ~ modData$death.rate + modData$pos.test.rate + modData$hosp.rate + modData$data.all.risk.med + modData$data.all.risk.mon + modData$data.all.age + modData$political.typology + modData$religiosity + modData$income.last.year + modData$household.earnings)
